...
void Arduino_RPiPicoSPI::writePixelsFast(uint16_t * data, uint32_t len) {
  WRITEBUF16(data,len);
}
...
INLINE void Arduino_RPiPicoSPI::WRITEBUF16(uint16_t *buf, size_t count)
{
  uint8_t buffer[count*2];
  for (uint i = 0; i< count; i++){
    buffer[i * 2] =(uint8_t)(buf[i]>>8);
    buffer[i * 2+1] =(uint8_t)(buf[i]&0x00ff);
  }
  spi_write_blocking(_spi, (const uint8_t *)buffer, count*2);
